...
@section('container')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Penggunaan Kontrasepsi Page</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="/dashboard">Data Master</a></li>
                        <li class="breadcrumb-item active">Penggunaan Kontrasepsi</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formData" name="formData">
                        <input type="hidden" name="id_penggunaan_kontrasepsi" id="id_penggunaan_kontrasepsi">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                <div class="form-group row">
                                    <label class="col-sm-3 col-form-label">Alat Kontrasepsi</label>
                                    <div class="col-sm-9">
                                        <select class="form-control" id="id_kontrasepsi" name="id_kontrasepsi">
                                            <option>Pilih Alat Kontrasepsi...</option>
                                            @foreach ($alatKontrasepsi as $data)
                                                <option value="{{ $data->id_kontrasepsi }}">{{ $data->nama_kontrasepsi }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
    ...
    <script>
        $(function() {
            var table = $('.data-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('penggunaan-kontrasepsi.index') }}",
            ...
            $('#addBtn').click(function() {
                ...
                $('#modelHeading').html("Create New Data Penggunaan Kontrasepsi");
            ...
            $('#saveBtn').click(function(e) {
                ...
                $.ajax({
                    url: "{{ route('penggunaan-kontrasepsi.store') }}",
                    data: {
                        "_token": "{{ csrf_token() }}",
            ...
            $('body').on('click', '.editData', function() {
                var id_penggunaan_kontrasepsi = $(this).data('id_penggunaan_kontrasepsi');
                $.ajax({
                    type: "POST",
                    url: "{{ route('penggunaan-kontrasepsi.edit') }}",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        id_penggunaan_kontrasepsi: id_penggunaan_kontrasepsi
                    },
                    dataType: 'json',
                    success: function(data) {
                        $('#modelHeading').html("Edit Penggunaan Kontrasepsi");
                        $('#saveBtn').val("edit-data");
                        $('#formModel').modal('show');
                        $('#id_penggunaan_kontrasepsi').val(data.id_penggunaan_kontrasepsi);
